...
class ThymioController:
    ...
    def __init__(self):
        ...
        # initialize the node
        rospy.init_node(
            'cnn_controller'  # name of the node
        )
        ...
        # load NN model
        self.angleness, self.centerness = None, None
        model_path = rospkg.RosPack().get_path('learning_to_drive') + '/saved_models/model_MobileNet_best.h5'
    ...
    def image_callback(self, data):
        ...
        try:
            ...
            # preprocess image
            im = cv2.resize(cv_image, (ThymioController.IM_W, ThymioController.IM_H), interpolation = cv2.INTER_AREA)
            im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB) / 255.

            # get predictions
            self.obstacle, self.angleness, self.centerness = self.model.predict(im[np.newaxis, :, :, :])

            print(self.obstacle, self.angleness, self.centerness)
    ...
    def run(self):
        """Controls the Thymio."""
        while (not rospy.is_shutdown()) and (self.angleness is None):
        	self.stop()
